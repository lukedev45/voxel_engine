...
// Face directions: 0=front, 1=back, 2=left, 3=right, 4=bottom, 5=top
// Each face has a normal offset to check if the neighbour is solid
...
// Vertices for each face of a unit cube, counter-clockwise winding
...
Chunk::Chunk(glm::ivec3 position) : position(position), VAO(0), VBO(0), vertexCount(0) {
    ...
    // Simple flat terrain — fill bottom half with stone
...
void Chunk::addFace(std::vector<float>& vertices, float x, float y, float z, int face) {
    ...
    // We store indices into FACE_VERTICES to form the two triangles
        ...
        for (int y = 0; y < CHUNK_SIZE; y++) {
            for (int z = 0; z < CHUNK_SIZE; z++) {
                ...
                for (int face = 0; face < 6; face++) {
                    ...
                    // Only add face if neighbour is air (face culling)
